...
RUN git clone https://github.com/laramaktub/plant-classification-tf-train.git && \
    cd plant-classification-tf-train && \
    pip install -e . && \
    cd ..

# TODO(aloga): use PyPi whenever possible
RUN git clone https://github.com/laramaktub/mydeepaas && \
    cd mydeepaas && \
    pip install -U . && \
    cd ..
...
RUN curl -o ./plant-classification-tf-train/plant_classification/training_weights/${THEANO_TR_WEIGHTS} \
    ${SWIFT_CONTAINER}${THEANO_TR_WEIGHTS}

RUN curl -o ./plant-classification-tf-train/plant_classification/training_info/${THEANO_TR_JSON} \
    ${SWIFT_CONTAINER}${THEANO_TR_JSON}

RUN curl -o ./plant-classification-tf-train/data/data_splits/synsets.txt \
...
RUN  cd /srv/plant-classification-tf-train/plant_classification/data/pretrained_weights/ &&  wget https://s3.amazonaws.com/lasagne/recipes/pretrained/imagenet/resnet50.pkl 
#RUN python /srv/plant-classification-tf-train/mount_nextcloud.py
